...
| `shortcutBindings` | Data (JSON) | `InAppShortcutBindings.defaults` — 13 `ShortcutBinding { key, modifiers }` entries; all bare except undo=⌘Z (`modifiers = NSEvent.ModifierFlags.command.rawValue`). Includes `markReady` (default `f`, bare — draft→ready write action). Decoded with custom `Codable` that handles both the new object shape and the v1.25.0 legacy bare-string shape; undo bare-string → `.command` migration preserves ⌘Z for existing users; absent `markReady` field falls back to factory default `f`. |
