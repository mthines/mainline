...
# Load environment variables from ~/.config/mainline/.env (if it exists)
load_env() {
    local env_file="$HOME/.config/mainline/.env"
    if [ -f "$env_file" ]; then
        while IFS= read -r line || [ -n "$line" ]; do
            ...
            if [ -z "${!key:-}" ]; then
                # shellcheck disable=SC2163  # $line is a KEY=VALUE pair from the env file
                export "$line"
            fi
        done < "$env_file"
...
update_plist_version() {
    local version="${1#v}"  # Remove 'v' prefix
    local plist="$PROJECT_DIR/Mainline/Mainline/Info.plist"
...
find_tap_dir() {
    ...
    local sibling_tap="$PROJECT_DIR/../homebrew-mainline"
    ...
    local brew_tap
    brew_tap=$(brew --repository mthines/mainline 2>/dev/null)
...
update_cask() {
    local version="$1"
    local zip_path="$2"
    local cask_file="$PROJECT_DIR/Casks/mainline.rb"
    ...
    if [ -z "$TAP_DIR" ]; then
        log_error "Homebrew tap not found. Run: bash scripts/setup-tap.sh"
    fi

    local tap_cask="$TAP_DIR/Casks/mainline.rb"
    if [ ! -f "$tap_cask" ]; then
        log_error "Tap cask file not found at $tap_cask"
    fi

    cp "$cask_file" "$tap_cask"
    log_success "Synced Cask to homebrew-mainline tap at $TAP_DIR"
}
...
main() {
    ...
    local bump
    bump=$(determine_bump "$current_version")
    ...
    # Confirm with user
    read -r -p "Release ${new_version}? (Y/n) " reply
    echo ""
    if [[ "$reply" =~ ^[Nn]$ ]]; then
        log_warning "Aborted."
        exit 0
    ...
    if [ -n "$(git status --porcelain Casks/mainline.rb 2>/dev/null)" ]; then
        git add Casks/mainline.rb
        git commit -m "feat(cask): update Mainline to version ${new_version#v} with new SHA256 checksum"
    ...
    # Push to remote
    read -r -p "Push to remote and create GitHub release? (Y/n) " reply
    echo ""
    if [[ ! "$reply" =~ ^[Nn]$ ]]; then
        ...
        log_info "Updating homebrew-mainline tap at $TAP_DIR..."
        cd "$TAP_DIR"
        git add Casks/mainline.rb
        if ! git commit -m "feat(cask): update Mainline to ${new_version#v}"; then
